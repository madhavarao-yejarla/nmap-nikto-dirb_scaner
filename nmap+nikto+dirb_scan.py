...
for target in targets:
    ...
    # Run Dirb scan for each target
    try:
        dirb_output = subprocess.check_output(["dirb", "http://" + target])
        with open(output_dir + "/" + target + "_" + str(date.today()) + "_dirb.txt", "w") as f:
            f.write(dirb_output.decode())
        print("Dirb scan results saved in " + output_dir + "/" + target + "_" + str(date.today()) + "_dirb.txt")
    except Exception as e:
        print(f"Error running Dirb scan for {target}: {e}")
